...
import { FileText, Download, Edit, Trash2, Eye, Send, Plus, ChevronDown, ChevronRight } from 'lucide-react';
...
import universityLogo from '../../assets/image.png'; // Make sure this path is correct
...
const EntranceExamSide = () => {
  ...
  // Download paper
  const handleDownloadPaper = (paper, format) => {
    try {
  ...
  const generatePDF = (paper, fileName) => {
    try {
      ...
      doc.setFontSize(12);
      doc.text(`Date: ${formatDate(paper.date || paper.createdAt)}`, 20, 50);
      doc.text(`Time: ${paper.examTime || 1} Hours`, 20, 60);
      
      // Use the configured mark type, not the calculated one
      doc.text(`Max Marks: ${paper.totalMarks}`, 180, 60, { align: 'right' });
  ...
  const generateDOCX = (paper, fileName) => {
    try {
        ...
        sections: [
          {
            ...
            children: [
              ...
              ...processQuestionsForDOCX(paper.questions)
            ]
          }
        ]
      });
  ...
  const processQuestionsForDOCX = (questions) => {
    if (!questions || !Array.isArray(questions) || questions.length === 0) {
      return [new Paragraph({ text: "No questions available." })];
    }
  ...
  // Edit paper
  const handleEditPaper = (paperId, event) => {
    if (event) event.stopPropagation();
  ...
  const handleDeletePaper = async (paperId, event) => {
    ...
    try {
      setLoading(true);
      // Make sure to call the updated deletePaper function which uses correct endpoint
  ...
  const handleSendForApproval = async (paper, event) => {
    ...
    const confirmSend = window.confirm('Will you send for approval to admin?');
    if (!confirmSend) return;
    
    try {
      // Implement send for approval logic
      const result = await saveCompletedPaper(paper);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Entrance Exam Papers</h1>
